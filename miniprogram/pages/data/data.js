...
function buildTrend(records, days) {
    ...
    return dateKeys.reduce((points, dateKey) => {
...
function drawLineChart(bundle, points, series, unit, references) {
    ...
    const allValues = series.reduce((result, item) => result.concat(item.values), references.slice());
