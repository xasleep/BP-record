...
const STATUS_TEXT: Record<BloodPressureStatus | 'empty', string> = {
  normal: '正常',
  high: '偏高',
  low: '偏低',
  empty: '暂无',
...
function compact(values: NullableNumber[]): number[] {
...
function resolveStatus(systolic: number | null, diastolic: number | null): BloodPressureStatus | 'empty' {
...
function drawGrid(ctx: any, width: number, height: number, padding: { top: number; right: number; bottom: number; left: number }, ticks: number[], yFor: (value: number) => number): void {
...
function drawLabels(ctx: any, points: TrendPoint[], height: number, padding: { top: number; right: number; bottom: number; left: number }, visibleTicks: number[], yFor: (value: number) => number, xFor: (index: number) => number, labelStep: number): void {
...
function drawReferences(ctx: any, references: number[], padding: { top: number; right: number; bottom: number; left: number }, width: number, yFor: (value: number) => number): void {
  const chartWidth = width - padding.left - padding.right
...
function drawSeries(ctx: any, series: ChartSeries[], xFor: (index: number) => number, yFor: (value: number) => number): void {
  series.forEach((item) => {
...
function drawLineChart(bundle: CanvasBundle, points: TrendPoint[], series: ChartSeries[], options: ChartOptions): void {
  ...
  const yFor = (value: number) => padding.top + chartHeight - ((value - options.yMin) / valueRange) * chartHeight
...
Page({
  ...
  data: {
    ...
    averageSystolic: '--',
    averageDiastolic: '--',
    overviewStatus: 'empty' as BloodPressureStatus | 'empty',
  ...
  initCanvas() {
    ...
    query.exec((result) => {
      ...
      const bpResult = result[0] as CanvasNodeResult | undefined
      const hrResult = result[1] as CanvasNodeResult | undefined
